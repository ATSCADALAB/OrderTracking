...
namespace Service
{
    internal sealed class CalendarReportService : ICalendarReportService
    {
        ...
        public async Task<IEnumerable<CalendarUserKpiDto>> GetUserKpiReportAsync(DateTime startDate, DateTime endDate, string currentUserId)
        {
            ...
            string userRole = userRoles.FirstOrDefault() ?? "User"; // Default là User nếu No data role
            ...
            if (userRole == "User")
            {
                ...
                {
                    return new List<CalendarUserKpiDto>(); // User No data calendar
                }
            }
            ...
            foreach (var userCal in userCalendarDto)
            {
                result.Add(new CalendarUserKpiDto
                {
                    ...
                    TotalPenalty = 0.0,
                    RewardOrPenalty = 0.0,
                    PenaltyDetails = "No data",
                    OvertimeDays = 0, // Khởi tạo giá trị tăng ca
                    OvertimeDetails = "No data" // Khởi tạo chi tiết tăng ca
                });
            }
            ...
            var excluded = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "Holidays in Vietnam", "soft@atpro.com.vn"
            ...
            foreach (var cal in calendars)
            {
                ...
                if (!events.Any())
                {
                    userResult.PenaltyDetails = "No data";
                    continue;
                }

                // Xử lý events và tính toán KPI
                var orders = new List<CalendarReport>();
                decimal totalPC = 0; // ✅ THAY ĐỔI: từ totalPenalty thành totalPC
                var excludedEvents = new List<string>();
                var pcDetails = new List<string>(); // ✅ THAY ĐỔI: từ penaltyDetails thành pcDetails
                var userOvertimeDates = new List<DateTime>();
                foreach (var ev in events)
                {
                    ...
                    // ✅ THAY ĐỔI: So sánh với evEnd thay vì evStart
                    if (evEnd < startDate || evEnd > endDate)
                        continue;

                    var lastTimelineDate = GetLastTimelineDate(desc);
                    if (lastTimelineDate == null) continue;
                    var timelineSteps = ParseTimelineFromDescription(desc);
                    var eventOvertimeDates = timelineSteps
                .Where(step => step.IsOvertime && step.Date >= startDate && step.Date <= endDate)
                .Select(step => step.Date.Date) // Lấy ngày (không lấy giờ)
                .ToList();

                    // Thêm các ngày tăng ca của event này vào danh sách tổng của user
                    userOvertimeDates.AddRange(eventOvertimeDates);
                    var qcDate = lastTimelineDate.Value;
                    int delta = (qcDate - evEnd).Days;

                    // Tính stars và days
                    int stars = CalculateStarsFromConfig(delta, kpiConfig);
                    int days = (evEnd - evStart).Days;

                    // ✅ THAY ĐỔI: Tính PC cho event này thay vì penalty
                    decimal eventPC = ExtractPCFromDescription(ev.Description ?? "", kpiConfig);
                    totalPC += eventPC;

                    // Thêm thông tin PC detail
                    if (eventPC != 0) // ✅ PC có thể âm hoặc dương
                    {
                        pcDetails.Add($"{ev.Summary}: {eventPC}");
                    }
                ...
                if (orders.Any())
                {
                    ...
                    double pc = (double)totalPC; // ✅ THAY ĐỔI: từ penalty thành pc
                    double reward = CalculateRewardFromConfig((decimal)avgStars, (decimal)hssl, (decimal)pc, kpiConfig);
                    ...
                    userResult.TotalPenalty = Math.Round(pc, 2); // ✅ Hiển thị PC trong field này
                    userResult.RewardOrPenalty = Math.Round(reward, 2);
                    userResult.PenaltyDetails = pcDetails.Any()
                        ? string.Join("; ", pcDetails)
                        : "No data";
                    var distinctOvertimeDates = userOvertimeDates.Distinct().ToList();
                    userResult.OvertimeDays = distinctOvertimeDates.Count;
                    userResult.OvertimeDetails = distinctOvertimeDates.Any()
                        ? string.Join(", ", distinctOvertimeDates.Select(d => d.ToString("dd/MM")))
                        : "No data";
                }
                else
                {
        ...
        private DateTime? GetLastTimelineDate(string description)
        {
            ...
            try
            {
                ...
                {
                    // Nếu có bất kỳ step nào No data [v] → timeline chưa hoàn thành
        ...
        public async Task<byte[]> GenerateReportAsync(DateTime startDate, DateTime endDate, string currentUserId)
        {
            ...
            // Lấy danh sách user calendar và filter theo role
            var userCalendarList = (await _repository.UserCalendar.GetUserCalendarsAsync(false)).ToList();
            var userCalendarDto = _mapper.Map<IEnumerable<UserCalendarDto>>(userCalendarList);
            ...
            if (userRole == "User")
            {
                ...
                {
                    var currentUserDto = _mapper.Map<UserCalendarDto>(currentUserCalendar);
                    userCalendarDto = new[] { currentUserDto };
                }
                else
                {
                    // Trả về file excel trống nếu user No data calendar
                    userCalendarDto = Enumerable.Empty<UserCalendarDto>();
                }
            }

            // Xác thực và khởi tạo Google Calendar Service
            UserCredential credential;
            using (var stream = new FileStream(Path.Combine(_googleAuthPath, "credentials.json"), FileMode.Open, FileAccess.Read))
            {
                credential = await GoogleWebAuthorizationBroker.AuthorizeAsync(
            ...
            var excluded = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "Holidays in Vietnam", "soft@atpro.com.vn"
            ...
            // Tạo sheet cho mỗi user trong danh sách đã filter
            foreach (var userCal in userCalendarDto)
            {
                ...
                // Trường hợp user No data calendar tương ứng hoặc No data events
                if (correspondingCalendar == null)
                {
                    ...
                    sheet.Cell(6, 1).Value = "Total PC";
                    sheet.Cell(6, 2).Value = 0;
                    sheet.Cell(7, 1).Value = "Bonus/Penalty";
                    sheet.Cell(7, 2).Value = 0;
                    sheet.Cell(8, 1).Value = "Total Overtime Days";
                    sheet.Cell(8, 2).Value = 0;
                    sheet.Cell(9, 1).Value = "List of Overtime Days";
                    sheet.Cell(9, 2).Value = "";

                    sheet.Cell(11, 1).Value = "Note";
                    sheet.Cell(11, 2).Value = "No Data";

                    var summaryzRange = sheet.Range("A1:B9");
                ...
                foreach (var ev in events)
                {
                    ...
                    // Tính ngày bắt đầu được tính tăng ca cho event này
                    var overtimeStartDate = CalculateOvertimeStartDate(evStart, evEnd);

                    // Trích xuất thông tin tăng ca từ timeline của event
                    var timelineSteps = ParseTimelineFromDescription(desc);
                    
                    // Lấy tất cả các ngày có status 'V' từ timeline theo thứ tự thời gian
                    var completedSteps = timelineSteps
                        .Where(step => !string.IsNullOrEmpty(step.Status) && 
                               step.Status.ToUpper() == "V" &&
                               step.Date.DayOfWeek != DayOfWeek.Sunday) // Loại bỏ chủ nhật
                        .OrderBy(t => t.Date)
                        .ToList();
                    
                    if (!completedSteps.Any()) continue;

                    // Ngày hoàn thành là ngày cuối cùng có status 'V' (không phải chủ nhật)
                    var completionDate = completedSteps.Last().Date;
                    
                    // Tính điểm 2/3 thời gian đơn hàng (đã trừ chủ nhật)
                    var totalWorkDays = completedSteps.Count;
                    
                    // Debug information
                    Console.WriteLine("Timeline analysis:");
                    foreach (var step in completedSteps)
                    {
                        Console.WriteLine($"- {step.Date:dd/MM/yyyy}");
                    }
                    
                    // Tính toán điểm 2/3 và log chi tiết
                    var twoThirdDays = Math.Ceiling(totalWorkDays * 2.0 / 3.0);
                    var twoThirdIndex = (int)twoThirdDays - 1; // -1 vì index bắt đầu từ 0
                    var twoThirdDate = completedSteps[twoThirdIndex].Date;
                    
                    Console.WriteLine($"Total work days (excluding Sundays): {totalWorkDays}");
                    Console.WriteLine($"Two-third calculation: {totalWorkDays} * 2/3 = {totalWorkDays * 2.0 / 3.0}");
                    Console.WriteLine($"Ceiling value: {twoThirdDays}, Index: {twoThirdIndex}");
                    Console.WriteLine($"Two-third point date: {twoThirdDate:dd/MM/yyyy}");

                    // Lọc các ngày tăng ca: phải từ điểm 2/3 trở đi và có status 'V', không tính chủ nhật
                    var markedOvertimeDates = timelineSteps
                        .Where(step => 
                            step.IsOvertime && // Có đánh dấu tăng ca
                            step.Date >= startDate && // Trong khoảng thời gian report
                            step.Date <= endDate &&
                            /*step.Date >= twoThirdDate &&*/ // Phải từ điểm 2/3 trở đi
                            step.Date.DayOfWeek != DayOfWeek.Sunday && // Không phải chủ nhật
                            !string.IsNullOrEmpty(step.Status) && step.Status.ToUpper() == "V") // Chỉ lấy những ngày đã hoàn thành
                        .OrderBy(step => step.Date)
                        .ToList();
                        
                    Console.WriteLine($"Total work days: {totalWorkDays}, Two-third point: {twoThirdDate:dd/MM/yyyy}");

                    // Kiểm tra tính liên tục của các ngày tăng ca
                    var eventOvertimeDates = new List<DateTime>();
                    if (markedOvertimeDates.Any())
                    {
                        // Lấy ngày tăng ca đầu tiên và kiểm tra ngày cuối
                        var hasOvertimeAtEnd = markedOvertimeDates.Any(d => d.Date == completionDate);
                        var firstOvertimeDate = markedOvertimeDates.Min(d => d.Date);

                        Console.WriteLine($"First overtime date: {firstOvertimeDate:dd/MM/yyyy}");
                        Console.WriteLine($"Two-third date: {twoThirdDate:dd/MM/yyyy}");
                        Console.WriteLine($"Has overtime at end: {hasOvertimeAtEnd}");

                        if (hasOvertimeAtEnd) // Bắt buộc phải có tăng ca ở ngày cuối
                        {
                            var consecutiveDates = new List<DateTime>();
                            DateTime startDate1;

                            // Xác định điểm bắt đầu kiểm tra tính liên tục
                            if (firstOvertimeDate > twoThirdDate)
                            {
                                // Nếu bắt đầu sau điểm 2/3 -> phải bắt đầu đúng ngày 2/3
                                Console.WriteLine("Started after 2/3 point - must start exactly at 2/3 point");
                                startDate1 = twoThirdDate;
                            }
                            else
                            {
                                // Nếu bắt đầu trước điểm 2/3 -> bắt đầu từ ngày tăng ca đầu tiên
                                Console.WriteLine("Started before 2/3 point - starting from first overtime date");
                                startDate1 = firstOvertimeDate;
                            }

                            Console.WriteLine($"Finding last continuous overtime period to completion date {completionDate:dd/MM/yyyy}");
                            
                            // Tìm chuỗi ngày tăng ca liên tục cuối cùng đến ngày kết thúc
                            var currentDate = completionDate;
                            var tempConsecutiveDates = new List<DateTime>();

                            // Đi ngược từ ngày cuối về trước để tìm chuỗi liên tục
                            while (currentDate >= startDate1)
                            {
                                var workingStep = completedSteps.FirstOrDefault(d => d.Date == currentDate);
                                if (workingStep == null) break;

                                var overtimeStep = markedOvertimeDates.FirstOrDefault(d => d.Date == currentDate);
                                if (overtimeStep == null)
                                {
                                    // Nếu gặp ngày không tăng ca, dừng lại - đã tìm được chuỗi liên tục cuối cùng
                                    Console.WriteLine($"Found break at {currentDate:dd/MM/yyyy}, stopping backwards search");
                                    break;
                                }

                                tempConsecutiveDates.Add(currentDate);
                                Console.WriteLine($"Added overtime day (searching backwards): {currentDate:dd/MM/yyyy}");

                                // Tìm ngày làm việc trước đó
                                var previousWorkingDate = completedSteps
                                    .Where(d => d.Date < currentDate)
                                    .OrderByDescending(d => d.Date)
                                    .FirstOrDefault();

                                if (previousWorkingDate == null) break;
                                currentDate = previousWorkingDate.Date;
                            }

                            // Đảo ngược lại danh sách để có thứ tự từ sớm đến muộn
                            tempConsecutiveDates.Reverse();

                            // Kiểm tra điều kiện tăng ca hợp lệ
                            if (tempConsecutiveDates.Any() && tempConsecutiveDates.Last() == completionDate)
                            {
                                var startOvertimeDate = tempConsecutiveDates[0];
                                
                                // Nếu ngày bắt đầu tăng ca nằm sau điểm 2/3
                                if (startOvertimeDate > twoThirdDate)
                                {
                                    // Phải bắt đầu đúng từ điểm 2/3 và có ít nhất 2 ngày liên tục
                                    if (startOvertimeDate == twoThirdDate && tempConsecutiveDates.Count >= 2)
                                    {
                                        Console.WriteLine($"Valid overtime period after 2/3 point from {tempConsecutiveDates[0]:dd/MM/yyyy} to {completionDate:dd/MM/yyyy}");
                                        eventOvertimeDates.AddRange(tempConsecutiveDates);
                                    }
                                    else
                                    {
                                        Console.WriteLine($"Invalid overtime: Started after 2/3 point ({twoThirdDate:dd/MM/yyyy}) at {startOvertimeDate:dd/MM/yyyy}");
                                    }
                                }
                                else
                                {
                                    // Nếu bắt đầu trước hoặc đúng điểm 2/3, chỉ cần đảm bảo có ít nhất 2 ngày liên tục
                                    if (tempConsecutiveDates.Count >= 2)
                                    {
                                        Console.WriteLine($"Valid continuous overtime period found from {tempConsecutiveDates[0]:dd/MM/yyyy} to {completionDate:dd/MM/yyyy}");
                                        eventOvertimeDates.AddRange(tempConsecutiveDates);
                                    }
                                    else
                                    {
                                        Console.WriteLine($"Invalid overtime: Only single day marked");
                                    }
                                }
                            }
                        }
                        ...
                        {
                            Console.WriteLine("No overtime on completion date - ignored");
                        }
                    }
                    userOvertimeDates.AddRange(eventOvertimeDates);
                ...
                double pc = (double)totalPC;
                double reward = orders.Any() ? CalculateRewardFromConfig((decimal)avgStars, (decimal)hssl, (decimal)pc, kpiConfig) : 0;

                // Tính toán kết quả tăng ca
                var distinctOvertimeDates = userOvertimeDates.Distinct().ToList();
                int overtimeDays = distinctOvertimeDates.Count;
                string overtimeDetails = distinctOvertimeDates.Any()
                    ? string.Join(", ", distinctOvertimeDates.Select(d => d.ToString("dd/MM")))
                    : "No data";

                // Điền dữ liệu tóm tắt vào sheet
                sheet.Cell(1, 1).Value = "Average Stars";
                ...
                sheet.Cell(6, 1).Value = "Total PC";
                sheet.Cell(6, 2).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
                sheet.Cell(6, 2).Value = Math.Round(pc, 2).ToString("#,##0");
                sheet.Cell(7, 1).Value = "Bonus/Penalty";
                var bonusPenaltyCell = sheet.Cell(7, 2);
                bonusPenaltyCell.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
                bonusPenaltyCell.Value = Math.Round(reward, 0); // Lưu giá trị số
                bonusPenaltyCell.Style.NumberFormat.Format = "#,##0"; // Áp dụng format sau
                sheet.Cell(8, 1).Value = "Total Allowance";
                sheet.Cell(8, 2).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;

                // Điền bảng chi tiết các đơn hàng
                sheet.Cell(11, 1).Value = "Title";
                sheet.Cell(11, 2).Value = "Start";
                sheet.Cell(11, 3).Value = "End";
                sheet.Cell(11, 4).Value = "QCDay";
                sheet.Cell(11, 5).Value = "Days";
                sheet.Cell(11, 6).Value = "Stars";
                sheet.Cell(11, 7).Value = "PC";
                ...
                foreach (var o in orders)
                {
                    ...
                    var pcCell = sheet.Cell(row, 7);
                    pcCell.Value = o.PC;
                    pcCell.Style.NumberFormat.Format = "#,##0";
                    pcCell.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;
                    row++;
                }

                // Add overtime details table on the right (column I)
                sheet.Cell(1, 9).Value = "OVERTIME DETAILS";
                sheet.Cell(2, 9).Value = "Date";
                sheet.Cell(2, 10).Value = "Amount";

                var overtimeRow = 3;
                var firstDataRow = overtimeRow; // Lưu vị trí dòng đầu tiên để dùng trong công thức

                // Add details for each overtime day
                foreach (var overtimeDate in distinctOvertimeDates.OrderBy(d => d))
                {
                    sheet.Cell(overtimeRow, 9).Value = overtimeDate.ToString("dd/MM/yyyy");
                    
                    // Format cột Amount để nhập số tiền
                    var amountCell = sheet.Cell(overtimeRow, 10);
                    amountCell.Value = ""; // Leave amount empty as requested
                    amountCell.Style.NumberFormat.Format = "#,##0";  // Format số có dấu phẩy ngăn cách hàng nghìn
                    overtimeRow++;
                }

                // Add total overtime amount row with formula
                var totalRow = overtimeRow;
                sheet.Cell(totalRow, 9).Value = "Total Amount:";
                
                // Thêm công thức SUM hoặc giá trị 0 cho ô tổng tiền
                var totalCell = sheet.Cell(totalRow, 10);
                if (distinctOvertimeDates.Any())
                {
                    totalCell.FormulaA1 = $"=SUM(J{firstDataRow}:J{totalRow-1})";
                }
                else
                {
                    totalCell.Value = 0;
                }
                totalCell.Style.NumberFormat.Format = "#,##0"; // Format số có dấu phẩy ngăn cách hàng nghìn
                
                // Bold dòng tổng
                sheet.Range(totalRow, 9, totalRow, 10).Style.Font.Bold = true;

                // Set the Total Allowance formula with proper formatting
                var totalAllowanceCell = sheet.Cell(8, 2);
                if (distinctOvertimeDates.Any())
                {
                    totalAllowanceCell.FormulaA1 = $"=B7+J{totalRow}";
                }
                else
                {
                    totalAllowanceCell.FormulaA1 = "=B7+0";
                }
                totalAllowanceCell.Style.NumberFormat.Format = "#,##0";

                // Format bảng tăng ca
                var overtimeHeaderRange = sheet.Range(1, 9, 2, 10);
                overtimeHeaderRange.Style.Fill.BackgroundColor = XLColor.LightGray;
                overtimeHeaderRange.Style.Font.Bold = true;
                overtimeHeaderRange.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                overtimeHeaderRange.Style.Border.OutsideBorder = XLBorderStyleValues.Thin;
                overtimeHeaderRange.Style.Border.InsideBorder = XLBorderStyleValues.Thin;

                var overtimeTableRange = sheet.Range(1, 9, overtimeRow, 10);
                overtimeTableRange.Style.Border.OutsideBorder = XLBorderStyleValues.Thin;
                overtimeTableRange.Style.Border.InsideBorder = XLBorderStyleValues.Thin;

                // Căn phải cột số tiền
                var moneyColumn = sheet.Range(3, 10, overtimeRow, 10);
                moneyColumn.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;

                // Format bảng đơn hàng
                sheet.Columns().AdjustToContents();
                // Format summary range without the last row
                var summaryRange = sheet.Range("A1:B8");
                ...
                // Format row 9 separately (no borders and no background color)
                var row9Range = sheet.Range("A9:B9");
                row9Range.Style.Font.Bold = true;

                var headerRange = sheet.Range("A11:G11");
                ...
                if (orders.Any())
                {
                    ...
                    var tableRange = sheet.Range($"A11:G{lastRow}");
                    tableRange.Style.Border.OutsideBorder = XLBorderStyleValues.Thin;
                    tableRange.Style.Border.InsideBorder = XLBorderStyleValues.Thin;

                    var starsRange = sheet.Range($"F12:F{lastRow}");
                    ...
                    var titleRange = sheet.Range($"A12:A{lastRow}");
                ...
                foreach (var pattern in patterns)
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        else if (match.Groups.Count == 2) // Chỉ có 1 phần
                        {
                            ...
                            // Nếu No data dấu gạch ngang hoặc chỉ có 1 phần, coi như là Title
                            return !string.IsNullOrWhiteSpace(part) ? part : "No data";
                        }
                    }
                }
            }
        ...
        // ✅ THAY ĐỔI CHÍNH: Method mới để extract PC thay vì PB
        private decimal ExtractPCFromDescription(string description, KpiConfiguration kpiConfig)
        {
            if (string.IsNullOrWhiteSpace(description))
                return 0; // Mặc định No data PC

            try
            {
                // Chuẩn hóa HTML content
                var plainText = CleanHtmlContentNoPC(description);

                // Tìm pattern PC:số (có thể có Note) - hỗ trợ cả số âm và dương
                var pcPattern = @"PC\s*:\s*(-?\d+(?:\.\d+)?)(?:\s*\([^)]*\))?";
                var match = Regex.Match(plainText, pcPattern, RegexOptions.IgnoreCase);

                if (match.Success && decimal.TryParse(match.Groups[1].Value, out decimal pcValue))
                {
                    // ✅ Lấy giá trị PC trực tiếp và nhân với 1000 (vì là hàng nghìn)
                    // PC có thể âm (PC:-200 = -200k) hoặc dương (PC:200 = +200k)
                    return pcValue * 1000;
                }

                // Nếu không tìm thấy PC, mặc định 0
                return 0;
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error extracting PC from description: {ex.Message}");
                return 0;
            }
        }
        ...
        private string ExtractQCFromDescription(string description)
        {
            ...
            // Pattern QC với ngày nhưng No data dấu gạch - ngoặc vuông
            @"\d{1,2}\/\d{1,2}\/\d{2,4}\s*:\s*QC\s*[-:]?\s*([^[\]\(\)\n\r;]+?)\s*\[([vV✓])\]",
            
            // Pattern QC với ngày nhưng No data dấu gạch - ngoặc tròn  
            @"\d{1,2}\/\d{1,2}\/\d{2,4}\s*:\s*QC\s*[-:]?\s*([^[\]\(\)\n\r;]+?)\s*\(([vV✓])\)",
            
            // Pattern QC No data ngày - ngoặc vuông
            @"QC\s*[-:]?\s*([^[\]\(\)\n\r;]+?)\s*\[([vV✓])\]",
            
            // Pattern QC No data ngày - ngoặc tròn
            @"QC\s*[-:]?\s*([^[\]\(\)\n\r;]+?)\s*\(([vV✓])\)",
            ...
            // Pattern QC chỉ có tên No data status
            @"QC\s*[-:]?\s*([^[\]\(\)\n\r;:]+?)(?:\s*[;:]?\s*$|\s*[;:])",
            
            // Pattern QC trong timeline với status hỗn hợp
            @"[-\s]*\d{1,2}\/\d{1,2}\/\d{2,4}.*?QC\s*[-:]?\s*([^[\]\(\)\n\r;]+?)(?:\s*[\[\(]([vV✓])[\]\)])?",
            
            // Pattern QC No data định dạng chuẩn
        ...
        // Helper method để tính ngày bắt đầu được tính tăng ca
        private DateTime CalculateOvertimeStartDate(DateTime startDate, DateTime endDate)
        {
            int totalDays = (endDate - startDate).Days + 1; // +1 vì bao gồm cả ngày cuối
            
            // Nếu đơn hàng chỉ có 1 ngày
            if (totalDays <= 1)
                return endDate;

            // Tính điểm bắt đầu 2/3 thời gian
            int daysBeforeOvertime = (int)Math.Floor(totalDays * (2.0 / 3.0));
            
            // Đảm bảo ít nhất là ngày cuối cùng với đơn hàng 2-3 ngày
            if (totalDays <= 3)
                return endDate;
                
            return startDate.AddDays(daysBeforeOvertime);
        }
        ...
        private List<string> SplitTimelineEntries(string timelineText)
        {
            ...
            try
            {
                ...
                // Bước 4: Nếu chỉ có 1 dòng hoặc No data line breaks, dùng regex pattern
        ...
        private TimelineStep? ParseTimelineEntryDirect(string entry)
        {
            ...
            try
            {
                // THAY ĐỔI: Cập nhật các pattern để có thể bắt được tag [tc] (tùy chọn) ở cuối dòng
                var patterns = new[]
                {
            // Pattern chính với ngoặc vuông và có thể có [tc]
            @"^-?\s*(\d{1,2}\/\d{1,2}\/\d{2,4})\s*:\s*(.*?)(\[([vVxX✓❌])\])?\s*(\[tc\])?\s*$",
            
            // Pattern với ngoặc tròn và có thể có [tc]
            @"^-?\s*(\d{1,2}\/\d{1,2}\/\d{2,4})\s*:\s*(.*?)(\(([vVxX✓❌])\))?\s*(\[tc\])?\s*$",

            // Pattern hỗn hợp và có thể có [tc]
            @"^-?\s*(\d{1,2}\/\d{1,2}\/\d{2,4})\s*:\s*(.*?)[\[\(]([vVxX✓❌])[\]\)]\s*(\[tc\])?\s*$"
        };
                ...
                foreach (var pattern in patterns)
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        // THAY ĐỔI: Kiểm tra sự tồn tại của group bắt tag [tc]
                        // Index của group này có thể khác nhau tùy vào pattern, chúng ta sẽ tìm group cuối cùng có giá trị "[tc]"
                        bool isOvertime = false;
                        for (int i = match.Groups.Count - 1; i > 2; i--)
                        {
                            if (match.Groups[i].Value.Equals("[tc]", StringComparison.OrdinalIgnoreCase))
                            {
                                isOvertime = true;
                                break;
                            }
                        ...
                        if (parsedDate.HasValue)
                        {
                            return new TimelineStep
                            {
                                ...
                                Status = status,
                                IsOvertime = isOvertime // Gán giá trị tăng ca
                            };
                        }
                    }
                }
        ...
        // ✅ THAY ĐỔI CHÍNH: Cập nhật công thức tính reward với PC
        private double CalculateRewardFromConfig(decimal averageStars, decimal hssl, decimal pc, KpiConfiguration config)
        {
            decimal reward = 0;

            if (averageStars >= config.Reward_HighPerformance_MinStars && averageStars <= config.Reward_HighPerformance_MaxStars)
            {
                // Công thức thưởng: ((TD-3)*2500000/2-500000) * HSSL + PC
                reward = ((averageStars - config.Penalty_MediumPerformance_MinStars) * config.Reward_BaseAmount / 2 - config.Reward_BasePenalty) * hssl + pc;
            }
            else if (averageStars >= config.Penalty_MediumPerformance_MinStars && averageStars < config.Penalty_MediumPerformance_MaxStars)
            {
                // Công thức phạt nhẹ: ((TD-3)*2500000/2-500000) * HSSL + PC
                reward = ((averageStars - config.Penalty_MediumPerformance_MinStars) * config.Reward_BaseAmount / 2 - config.Reward_BasePenalty) * hssl + pc;
            }
            else if (averageStars >= config.Penalty_LowPerformance_MinStars && averageStars < config.Penalty_LowPerformance_MaxStars)
            {
                // Công thức phạt nặng: ((TD-1)*500000/2-1000000) * HSSL + PC
                reward = ((averageStars - config.Penalty_LowPerformance_MinStars) * config.Penalty_LowPerformance_BaseAmount / 2 - config.Penalty_LowPerformance_MaxPenalty) * hssl + pc;
            }
        ...
        // ✅ THAY ĐỔI: Method helper để clean HTML content và loại bỏ PC
        private string CleanHtmlContent(string htmlContent)
        {
            ...
            // ✅ THAY ĐỔI: Loại bỏ phần PC khỏi hiển thị timeline (thay vì PB)
            plainText = Regex.Replace(plainText, @"PC\s*:\s*-?\d+(?:\s*\([^)]*\))?\s*", "", RegexOptions.IgnoreCase);
        ...
        // ✅ THAY ĐỔI: Method helper mới để clean HTML content nhưng không loại bỏ PC
        private string CleanHtmlContentNoPC(string htmlContent)
        {
            if (string.IsNullOrWhiteSpace(htmlContent))
                return "";
        ...
        private string? ExtractTimelineSection(string content)
        {
            ...
            // Không có "timeline" hoặc không có nội dung sau đó
            return "";
        }
